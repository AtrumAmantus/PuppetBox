...
namespace PB
{
    namespace Event::Pipeline::Topic
    {
        ...
        std::uint32_t BONE_TRANSFORM_TOPIC = MessageBroker::instance().registerTopic(PB_EVENT_PIPELINE_BONE_TRANSFORM_TOPIC);
        std::uint32_t ENTITY_TRANSFORM_TOPIC = MessageBroker::instance().registerTopic(PB_EVENT_PIPELINE_ENTITY_TRANSFORM_TOPIC);
        std::uint32_t GET_ABS_BONE_TRANSFORM_TOPIC = MessageBroker::instance().registerTopic(PB_EVENT_PIPELINE_GET_ABS_BONE_TRANSFORM_TOPIC);
        std::uint32_t GET_BONE_TRANSFORM_TOPIC = MessageBroker::instance().registerTopic(PB_EVENT_PIPELINE_GET_BONE_TRANSFORM_TOPIC);
